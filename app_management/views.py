...
def get_store_data(request):
    ...
    if selected_province == 'all':
        ...
        Inqueryset = Inside.objects.select_related('store').values('pop_description','employee_no', 'store__site_name', 'point_of_sale','self_order_kiosk','promo_sok_campaigns','happy_m_campaign','description_inside')
        for item in Inqueryset:
            ...
            out_pop_description=item['pop_description']
...
def get_store_data_store_level(request):
    ...
    if month and "_" in month:
        ...
        dates_in_month = [datetime(int(year), month_number, day).date() for day in range(1, last_day + 1)]
    print('dates')
    ...
    if selected_store == 'all':
        ...
        Outqueryset = Outqueryset.filter(outside_date__in=dates_in_month)
        for item in Outqueryset:
            branding_condition_score = 1 if item['branding_condition'] else 0
            signage_condition_score = 1 if item['signage_condition'] else 0
            campaign_score = 1 if item['campaign'] else 0
        ...
        Inqueryset = Inside.objects.select_related('store').values('pop_description','employee_no', 'store__site_name', 'point_of_sale','self_order_kiosk','promo_sok_campaigns','happy_m_campaign','description_inside')
        Inqueryset = Inqueryset.filter(inside_date__in=dates_in_month)
        for item in Inqueryset:
            point_of_sale_score = 1 if item['point_of_sale'] else 0
            self_order_kiosk_score = 1 if item['self_order_kiosk'] else 0
        ...
        Mainqueryset = Menu.objects.select_related('store').values('employee_no', 'store__site_name', 'menu_visibility','price_visibility','menu_promotion','description_menu')
        Mainqueryset = Mainqueryset.filter(menu_date__in=dates_in_month)
        for item in Mainqueryset:
            menu_visibility_score = 1 if item['menu_visibility'] else 0
        ...
        Mccafequeryset = McCafe.objects.select_related('store').values('employee_no', 'store__site_name', 'menu_visibility','menu_promo','description_mccafe')
        Mccafequeryset = Mccafequeryset.filter(store__site_name=selected_store,mccafe_date__in=dates_in_month)
        for item in Mccafequeryset:
            mccafemenu_visibility_score = 1 if item['menu_visibility'] else 0
        ...
        drivethruqueryset = Drivethru.objects.select_related('store').values('employee_no', 'store__site_name', 'activation_on_promo','drivethru_campaign','customer_order_display','activation_description')
        drivethruqueryset = drivethruqueryset.filter(store__site_name=selected_store,mccafe_date__in=dates_in_month)
        for item in drivethruqueryset:
            activation_on_promo_score = 1 if item['activation_on_promo'] else 0
        ...
        deliveryqueryset = deliveryqueryset.filter(store__site_name=selected_store,delivery_date__in=dates_in_month)
        for item in deliveryqueryset:
            mc_delivery_score = 1 if item['mc_delivery'] else 0
            third_party_del_score = 1 if item['third_party_del'] else 0
        ...
        Outqueryset = Outqueryset1.filter(store__site_name=selected_store, outside_date__in=dates_in_month)
        for item in Outqueryset:
            branding_condition_score = 1 if item['branding_condition'] else 0
            signage_condition_score = 1 if item['signage_condition'] else 0
            campaign_score = 1 if item['campaign'] else 0
        ...
        Inqueryset = Inside.objects.select_related('store').values('pop_description','employee_no','inside_date','store__site_name', 'point_of_sale','self_order_kiosk','promo_sok_campaigns','happy_m_campaign','description_inside')
        Inqueryset = Inqueryset.filter(store__site_name=selected_store,inside_date__in=dates_in_month)
        for item in Inqueryset:
            point_of_sale_score = 1 if item['point_of_sale'] else 0
            self_order_kiosk_score = 1 if item['self_order_kiosk'] else 0
        ...
        Mainqueryset = Mainqueryset.filter(store__site_name=selected_store,menu_date__in=dates_in_month)
        for item in Mainqueryset:
            menu_visibility_score = 1 if item['menu_visibility'] else 0
        ...
        drivethruqueryset = drivethruqueryset.filter(store__site_name=selected_store,drivethru_date__in=dates_in_month)
        for item in drivethruqueryset:
            activation_on_promo_score = 1 if item['activation_on_promo'] else 0
